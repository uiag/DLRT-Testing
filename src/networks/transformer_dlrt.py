...
class DecoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, x, enc_output, training, look_ahead_mask, padding_mask, step):
        ...
        attn1, attn_weights_block1 = self.mha1(x, x, x, look_ahead_mask, step)  # (batch_size, target_seq_len, d_model)
        attn1 = self.dropout1(attn1, training=training)
        out1 = self.layernorm1(attn1 + x)

        attn2, attn_weights_block2 = self.mha2(enc_output, enc_output, out1, padding_mask,
                                               step)  # (batch_size, target_seq_len, d_model)
    ...
    def call(self, x, enc_output, training, look_ahead_mask, padding_mask, step):
        ...
        for i in range(self.num_layers):
            x, block1, block2 = self.dec_layers[i](x, enc_output, training,
                                                   look_ahead_mask, padding_mask, step)
...
class TransformerDLRT(tf.keras.Model):
    ...
    def call(self, inputs, training, step):
        ...
        dec_output, attention_weights = self.decoder(tar, enc_output, training, look_ahead_mask, padding_mask, step)
